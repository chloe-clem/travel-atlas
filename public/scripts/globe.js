(function(){
  ...
  let rotation=[-12,-18,0],dragging=false,last=[0,0],dpr=Math.min(devicePixelRatio||1,1.25),projection,path,width=0,height=0;
  ...
  function draw(t){const dt=Math.min(34,t-lastFrame);lastFrame=t;if(!dragging&&performance.now()-lastInteraction>1700)rotation[0]=(rotation[0]-.008*dt)%360;projection.rotate(rotation);ctx.clearRect(0,0,width,height);drawOcean();drawLand();drawNight();drawAtmosphere();requestAnimationFrame(draw)}
  function clamp(v){return Math.max(0,Math.min(1,v))}
  function smoothstep(a,b,x){x=clamp((x-a)/(b-a));return x*x*(3-2*x)}
  function updateStory(){const rect=story.getBoundingClientRect(),max=Math.max(1,story.offsetHeight-innerHeight),p=clamp(-rect.top/max);const title=1-smoothstep(.08,.34,p);const earth=1-smoothstep(.25,.55,p);const reveal=smoothstep(.48,.66,p);const revealY=(1-reveal)*28;const blur=smoothstep(.30,.58,p)*5;hero.style.setProperty('--story',p.toFixed(3));hero.style.setProperty('--title-opacity',title.toFixed(3));hero.style.setProperty('--earth-opacity',earth.toFixed(3));hero.style.setProperty('--reveal-opacity',reveal.toFixed(3));hero.style.setProperty('--reveal-y',revealY.toFixed(1)+'px');hero.style.setProperty('--earth-blur',blur.toFixed(2)+'px');nav&&nav.classList.toggle('scrolled',scrollY>80)}
